            ...
            axios
                ...
                .then(async (res) => {
                    ...
                    if (res.status === 200 && isListenerActive) {
                        ...
                        const cRes = await axios.get(`${FIRST_CONSUMER_URL}/?prefix=${sessionStorage.getItem("UID")}&topic=first-topic`);
                        if (cRes.status === 200) {
                            const items: string[] = cRes.data;
                            console.log("HERE ARE THE ITEMS: ", items);
                            /*
                            console.log("HERE IS cRes.data: ", cRes.data);
                            const decodedData = JSON.parse(cRes.data);
                            console.log("HERE IS decodedData: ", decodedData);
                            if (decodedData.length !== 0) {
                                decodedData.forEach((item: { [key: string]: string }) => {
                                    let str = "";
                            ...
                            setConsumerList((prev: string[]): string[] => [...items, ...prev]);
                        }
                    }
                })
                .catch((e) => {
