...
export default function Section3() {
    ...
    useEffect(() => {
            ...
            axios
                ...
                .then(async (res) => {
                    if (res.status === 200 && isListenerOneActive) {
                        ...
                        const cRes = await axios.get(`${BLUE_CONSUMER_URL}/?prefix=${sessionStorage.getItem("UID")}&topic=second-topic`);
                        if (cRes.status === 200) {
                            const items: string[] = cRes.data;
                            setFirstConsumerList((prev: string[]): string[] => [...items, ...prev]);
                        }
                    }
                })
                .catch(() => {
        ...
        const secondConsumerInt = setInterval(() => {
            axios
                ...
                .then(async (res) => {
                    if (res.status === 200 && (isListenerOrangeActive || isListenerTwoActive)) {
                        setSecondConsumerActive(true);
                        if (isListenerOrangeActive) { setSecondConsumerGroup("orange-group") } 
                        else { setSecondConsumerGroup("blue-group") }
                        let cRes: AxiosResponse<any, any>;
                        if (isListenerOrangeActive) {
                            cRes = await axios.get(`${ORANGE_CONSUMER_URL}/?prefix=${sessionStorage.getItem("UID")}&topic=second-topic`);
                        }
                        else {
                            cRes = await axios.get(`${FIRST_CONSUMER_URL}/?prefix=${sessionStorage.getItem("UID")}&topic=second-topic`);
                        }
                        if (cRes.status === 200) {
                            const items: string[] = cRes.data;
                            setSecondConsumerList((prev: string[]): string[] => [...items, ...prev]);
                        }
                    }
                })
                .catch(() => {
    ...
    }, [isProducerActive, isListenerOneActive, isListenerOrangeActive, isListenerTwoActive]);
    ...
    return (
        <>
            ...
            <Grid container direction="row" spacing={2} marginBottom={"1.5em"} alignItems="center">
                <Grid item sm={6}>
                    <Paper variant={"outlined"} square={false} style={{ padding: "1rem", marginTop: "2rem" }} className={"consumer"}>
                        ...
                        <Typography
                            ...
                            gutterBottom
                            fontWeight={"bold"}
                            color={!isFirstConsumerActive ? "lightgray" : firstConsumerGroup === "blue-group" ? "info.main" : "warning.main"}
                        >
                ...
                <Grid item sm={6}>
                    <Paper variant={"outlined"} square={false} style={{ padding: "1rem", marginTop: "2rem" }} className={"consumer"}>
                        ...
                        <Typography
                            ...
                            gutterBottom
                            fontWeight={"bold"}
                            color={!isSecondConsumerActive ? "lightgray" : (secondConsumerGroup === "blue-group" ? "info.main" : "warning.main")}
                        >
