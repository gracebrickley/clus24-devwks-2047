...
export default function Section4() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch(() => {
        console.log(
          "section 4 provisioner failed to consume from topic new-user"
        );
      });
    axios
      .post(`${PROVISIONER_URL}/start-consumer`, {
      ...
      .catch(() => {
        console.log(
          "section 4 provisioner failed to consume from topic notified"
        );
      });
    axios
      ...
      .catch(() => {
        console.log(
          "section 4 provisioner failed to produce to topic authorize"
        );
      });
    ...
    axios
      ...
      .catch(() => {
        console.log(
          "section 4 authorizer failed to consume from topic authorize"
        );
      });
    axios
    ...
    const sagaInt = setInterval(async () => {
      ...
      for (let i = 0; i < responses.length; i++) {
        ...
        if (val?.data?.length !== 0) {
          console.log(
            "HERE IS THE DATA when val?.data?.length !== 0: ",
            val.data
          );
      ...
      <Paper variant={"outlined"} square={false} style={{ padding: "1rem" }}>
        ...
        <Grid container direction="row" spacing={2} alignItems="center">
          ...
          <Grid item sm={4}>
            <FormGroup>
              <FormControlLabel
                control={<Switch defaultChecked onChange={toggleTraceMode} />}
                label="Trace Mode"
              />
            </FormGroup>
          </Grid>
